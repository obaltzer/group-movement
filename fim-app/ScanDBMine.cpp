...
int CScanDBMine::ScanDBCountFreqItems(ITEM_COUNTER **ppfreqitems)
{
	...
	int *pitem_sup_map, *ptemp_map, ncapacity, nnewcapacity;
	int i, k;
...
void CScanDBMine::ScanDBBuildCondDB(HEADER_TABLE pheader_table, int *pitem_order_map, int num_of_freqitems, int *Buckets)
{
	Transaction *ptransaction;
		...
		if(ptransaction->length>0)
		{
			...
			if(ntrans_len>1)
			{
				...
				if(order<num_of_countitems)
				{
					...
					{
						goAFOPTMiner.InsertTransaction(pheader_table, order, &(pproj_trans[1]), ntrans_len-1, 1);
					}
					else
					{
...
void CScanDBMine::ScanDBBucketCount(int *pitem_order_map, int *Buckets)
{
	Transaction *ptransaction;
	unsigned int k;
	int i;
