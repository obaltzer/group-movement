...
public:
	...
	int *mpbranch, *mpfrequencies, *mptransaction;
	...
	void InsertTransaction(HEADER_TABLE pheader_table, int nitem_pos, int* ptransaction, int length, int frequency);

	void InsertTransaction(AFOPT_NODE *ptreeroot, ARRAY_NODE* ptransaction, int length);
//	void InsertTransaction(AFOPT_NODE *proot, int* ptransaction, int length, int frequency);

	void DestroyTree(AFOPT_NODE * proot);
	int  TraverseTree(AFOPT_NODE * proot);

	void DepthAFOPTGrowth(HEADER_TABLE pheader_table, int num_of_freqitems, int nstart_pos);
	...
	void CountFreqItems(AFOPT_NODE* proot, int *pitem_sup_map);
	void BuildNewTree(AFOPT_NODE *proot, HEADER_TABLE pnewheader_table, int *pitem_order_map, int num_of_newfreqitems, int *Buckets);
	...
	void BucketCountAFOPT(AFOPT_NODE *proot, int *pitem_order_map, int *Buckets);
