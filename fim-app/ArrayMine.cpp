...
//-------------------------------------------------------------------------------
void CArrayMine::CountFreqItems(TRANS_HEAD_NODE* pcond_db, int *pitem_sup_map)
{
	...
	while(pthnode!=NULL)
	{
		...
		{
			pitem_sup_map[pthnode->ptransaction[i]]++;
		}
...
void CArrayMine::PushRight(HEADER_TABLE pheader_table, int item_pos, int nmax_push_pos)
{
	...
	while(pthnode!=NULL)
	{
		...
		if(nstart_pos<pthnode->length-1)
		{
			...
			{
				nstart_pos++;
				goAFOPTMiner.InsertTransaction(pheader_table, norder, &(pthnode->ptransaction[nstart_pos]), pthnode->length-nstart_pos, 1);
				delete []pthnode->ptransaction;
				//goMemTracer.DelOneTransaction(pthnode->length);
				delete pthnode;
				//goMemTracer.DelOneTransHeadNode();
...
//-------------------------------------------------------------------------------
void CArrayMine::BucketCount(TRANS_HEAD_NODE *pcond_db, int* pitem_order_map, int *Buckets)
{
	int i;
	unsigned int k;
	TRANS_HEAD_NODE *pthnode;
...
//-----------------------------------------------------------------------------------------
void CArrayMine::BuildNewCondDB(TRANS_HEAD_NODE *pcond_db, HEADER_TABLE pnewheader_table, int* pitem_order_map, int num_of_freqitems, int *Buckets)
{
	int i;
