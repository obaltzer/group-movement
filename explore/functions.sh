...
function group()
{
    ...
    if test "${algorithm}" != "both" ; then
        ...
        path="${target_dir}/${algorithm}/${ast}/${ost}/${support}/${min_length}"
    fi
    ...
    if test -s "${clname}" ; then
        ...
        if test "${algorithm}" == "attachment" ; then 
            runtime=`get_time ${BIN_PATH}/nc2 -t ${weight} -f ${finame} ${datname} ${clname} ${grpname}`
        elif test "${algorithm}" == "overlap" ; then 
            runtime=`get_time ${BIN_PATH}/wc2 -t ${weight} -f ${finame} -e ${empname} ${datname} ${clname} ${grpname}`
        elif test "${algorithm}" == "both" ; then 
            runtime=`get_time ${BIN_PATH}/both -a ${ast} -o ${ost} -f ${finame} -e ${empname} ${datname} ${clname} ${grpname}`
        fi
