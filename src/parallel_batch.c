...
int configure(parallel_config_t* config, int argc, char** argv)
{
    ...
    while((ch = getopt_long(argc, argv, "c:p:f:n:s:", longopts, NULL)) != -1)
    {
        switch(ch)
        {
            case 'c':
...
int substitude_parameters(char* dst, char* src, parameter_set_t params, int node_id)
{
    char* tmp;
    char* state;
    char* item;
    ...
    strcpy(tmp, src);
    dst[0] = '\0';
    snprintf(nid_buffer, 20, "%d", node_id);
    for(item = strtok_r(tmp, "%", &state); item; item = strtok_r(NULL, "%", &state))
    {
        if(item[0] != '\0')
        {
            if(item[0] == 'N')
            {
                strcat(dst, nid_buffer);
                item++;
            }
            else if((pi = item[0] - 0x30) < params.n_params && pi >= 0)
            {
...
int execute_command(char* command, parameter_set_t* params, int node_id)
{
    int retval;
    char cmd[1024];

    substitude_parameters(cmd, command, *params, node_id);
    retval = system(cmd);
...
int parameter_set_check_flag(char* flag_name, parameter_set_t* params, int node_id)
{
    char dst[256];
    struct stat stats;

    substitude_parameters(dst, flag_name, *params, node_id);
    if(stat(dst, &stats) == 0 && S_ISREG(stats.st_mode))
        return TRUE;
    return FALSE;
}

void parameter_set_flag(char* flag_name, parameter_set_t* params, int node_id)
{
    char dst[256];
    FILE* flag;
...
int main(int argc, char** argv)
{
    ...
    if(rank == 0)
    {
	    ...
	    range[0] = i;
            range[1] = i + config.batch_size <= psl->n_sets ? i + config.batch_size : psl->n_sets;
            printf("Sending batch [%d, %d] to node %d.\n", range[0], range[1], n);
            MPI_Send(range, 2, MPI_INT, n + 1, 0, MPI_COMM_WORLD);
            active_nodes[n]++;
            i = range[1];
        }
        ...
        while(!done)
        {
            ...
            {
	        MPI_Recv(&n, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
                printf("Received termination from: %d\n", n);
                active_nodes[n]--;
            }
            else
                done = 1;
	    ...
	    MPI_Recv(range, 2, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
            if(range[0] != -1)
            {
                ...
                {   
                    ...
                    {
                        if(!parameter_set_check_flag(config.flag_name, &psl->sets[i], rank))
                        {
                            if(execute_command(config.cmd, &psl->sets[i], rank) != 0)
                                parameter_set_flag(config.flag_name, &psl->sets[i], rank);
                        }   
                        else
                        {
                            parameter_set_print(&psl->sets[i]);
